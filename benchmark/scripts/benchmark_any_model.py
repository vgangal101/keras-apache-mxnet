...
parser.add_argument('--dataset',
                    help='Dataset for training: cifar10,imagenet,synthetic')
...
parser.add_argument('--batch_size',default=32,type=int,help='batch size to be used during training')
parser.add_argument('--model',default='resnet50',type=str,help='model to be run during training')
...
num_classes = 1000 if args.dataset == 'imagenet' or 'synthetic' else 10
data_format = K.image_data_format
# Subtracting pixel mean improves accuracy
subtract_pixel_mean = True

# prepare logging
# file name: backend_data_format_dataset_model_batch_size_gpui.log
log_file = K.backend() + '_' + K.image_data_format() + '_' + args.model + '_' + '_batch_size' + str(batch_size) + '_' + str(num_gpus) + 'gpus'  # nopep8
...
if args.dataset == 'synthetic':
    num_samples = 10000
    input_shape = (num_samples,224,224,3) if data_format == 'channels_last' else (num_samples,3,224,224)
...
#get the keras model application model here
...
rootLogger.info('Training using: ' + args.model)

# Prepare model saving directory.
save_dir = os.path.join(os.getcwd(), 'saved_models')
model_name = 'imagenet_%s_model.{epoch:03d}.h5' % args.model
